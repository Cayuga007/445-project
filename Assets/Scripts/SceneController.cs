...
public class SceneController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        LeanTween.alpha(black, 0, black_fade_duration);
    }
    public void LoadNextScene()
    {
        if (current_day < 4)
        {
            LeanTween.alpha(black, 1, black_fade_duration).setOnComplete(NextDay);
        } else
        {
            LeanTween.alpha(white, 1, white_fade_duration).setOnComplete(LastDay);
        }
    }
